...
function Logger (content) {
  var title = this.title = content.alert_title.replace(/\./g, ':')
    , date = this.date = new Date()
    , timestamp = this.timestamp = date.getTime()
    , bucket = this.bucket = Math.floor(this.timestamp/config.bucketLength) // This should create buckets at 5-minute intervals

    , data = this.data = content.data
    , dfds = this.dfds = [];
