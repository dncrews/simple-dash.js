...
function logger(content, next) {
  ...
  if (type === 'app') {
    ...
    dfds.push(logHerokuStatus()); //FIXME: does this go here? Placing here to ensure this happens only once every 5 min or so.
...
function createRawBucket(alertTitle, data, timeBucket) {
  ...
  db.mongo.rawBucket.findOne({ "timeBucket" : timeBucket }, function(err, doc) {
    doc = doc || { "timeBucket" : timeBucket, "created_at": new Date() }; //creates BSON timestamp (driver auto-converts) that allows mongo to auto-expire base on INDEX
    doc[alertTitle] = data;
...
//FIXME: we need to re-structure these files MAJORLY!!!
//get & log heroku status to DB
function logHerokuStatus(cb) {
  var dfd = Q.defer(); //set up q deferred obj
  ...
  request
    ...
    .end(function(res){
      //if any errors, abort
      if (!res.ok) return console.log('Oh no! error checking for heroku status: ' + res.text);
      ...
      db.mongo.upstream_status.save(doc, getAsyncResolve(dfd));
