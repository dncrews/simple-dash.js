/*global Buffer:false,clearInterval:false,clearTimeout:false,console:false,exports:false,global:false,module:false,process:false,querystring:false,require:false,setInterval:false,setTimeout:false,__filename:false,__dirname:false */
'use strict';

var Q = require('q');

var databaseUrl = process.env.databaseUrl || process.env.MONGOHQ_URL
  , config = require('./config')
  , collections = [ 'appBucket', 'apiStatus', 'rawBucket', 'rawStatus', 'apiMap', 'change_log', 'upstream_status' ]
  , mongo = require('mongojs').connect(databaseUrl, collections);

module.exports = {
  mongo : mongo,
  getApiRecent : getApiRecent,
  getAppApiRecent : getAppApiRecent,
  getRecent : getRecent,
  getAppBucket : getAppBucket,
  getAPIBucket : getAPIBucket,
  saveApiMap : saveApiMap,
  getApiMap : getApiMap,
  getRecentEvents: getRecentEvents
};


//TODO: this stuff should be somewhere else. Either that, or move the mogno config stuff above somewhere else...
//get events by app (deploy, restart, etc)
function getRecentEvents(appName) {
  var dfd = Q.defer()
    , appNames = [];

  //FIXME: this data-sanitization here feels dirty. Should we normalize this in the DB?
  //strip out the -prod|-test for the changelog
  var app_name_clean = appName.match(/(fs-\w+)/i)[1];
  //if it's a repo, just extract the 'photos'. Ignore suffix and prefix
  var repo_name = appName.match(/fs-(\w+)/i)[1];


  //find the app by repo_name OR app_name
  mongo.change_log.find({ $or: [{"data.repo_name": repo_name}, {"data.app_name": app_name_clean}] }).sort({ "timestamp" : -1}).limit(20, function(err, docs) {
    dfd.resolve(docs);
  }); //FIXME: should this be somewhere else?

  return dfd.promise;
}

function getApiRecent() {
  var dfd = Q.defer()
    , apiNames = []
    , day1 = new Date();

  day1.setDate(day1.getDate() - 2);
  day1 = day1.getTime();

  // Get all (200 most recent) time buckets for all apps
  mongo.apiStatus.find({timestamp:{$gte: day1}}).sort({ "timestamp" : -1, "api": 1 }).limit(200, function(err, docs) {
    // Remove all duplicate apiNames
    docs = docs.filter(function(el) {
      if (apiNames.indexOf(el.api) === -1) {
        apiNames.push(el.api);
        return true;
      }
      return false;
    });
    dfd.resolve(docs);
  });

  return dfd.promise;
}

function getAppApiRecent(appName) {
  var dfd = Q.defer()
    , apiNames = []
    , day1 = new Date();

  day1.setDate(day1.getDate() - 2);
  day1 = day1.getTime();

  getApiMap(appName).done(function(list) {
    mongo.apiStatus.find({timestamp:{$gte: day1}, api: { $in: list }}).sort({ "timestamp" : -1, "api": 1 }).limit(200, function(err, docs) {
      // Remove all duplicate apiNames
      docs = (docs || []).filter(function(el) {
        if (apiNames.indexOf(el.api) === -1) {
          apiNames.push(el.api);
          return true;
        }
        return false;
      });
      dfd.resolve(docs);
    });
  });

  return dfd.promise;
}

function getRecent() {
  var dfd = Q.defer()
    , appNames = []
    , day1 = new Date();

  day1.setDate(day1.getDate() - 2);
  day1 = day1.getTime();
  day1 = Math.floor(day1/config.bucketLength);

  // Get all (200 most recent) time buckets for all apps
  mongo.appBucket.find({timeBucket:{$gte: day1}}).sort({ "timeBucket" : -1, "appName": 1 }).limit(200, function(err, docs) {
    if (err) {
      console.warn(err);
      dfd.reject();
    }
    // Remove all duplicate appNames
    docs = docs && docs.filter(function(el) {
      if (appNames.indexOf(el.appName) === -1) {
        appNames.push(el.appName);
        return true;
      }
      return false;
    });
    dfd.resolve(docs);
  });

  return dfd.promise;
}

function getAppBucket(appName) {
  var dfd = Q.defer()
    , day1 = new Date();

  day1.setDate(day1.getDate() - 2);
  day1 = day1.getTime();
  day1 = Math.floor(day1/config.bucketLength);

  mongo.appBucket.find({ "appName" : appName, timeBucket:{$gte: day1} }).sort({ timeBucket : -1 }, function(err, docs) {
    if (err) {
      return dfd.reject(err);
    }
    dfd.resolve(docs);
  });
  return dfd.promise;
}

function getAPIBucket(apiName) {
  var dfd = Q.defer()
    , day1 = new Date();

  day1.setDate(day1.getDate() - 2);
  day1 = day1.getTime();
  mongo.apiStatus.find({ "api" : apiName, timestamp:{$gte: day1} }).sort({ timestamp : -1 }, function(err, docs) {
    if (err) {
      return dfd.reject(err);
    }
    dfd.resolve(docs);
  });
  return dfd.promise;
}

function saveApiMap(list) {
  var dfd = Q.defer();

  mongo.apiMap.findOne(function(err, doc) {
    var i, l, _rel, appName, apis;
    doc = doc || {};
    for (i=0, l=list.length; i<l; i++) {
      _rel = list[i];
      appName = _rel.fs_host;
      if (! doc[appName]) {
        doc[appName] = [];
      }
      apis = doc[appName];
      if (apis.indexOf(_rel.api) === -1) {
        apis.push(_rel.api);
      }
    }
    mongo.apiMap.save(doc, function() {
      dfd.resolve();
    });
    dfd.resolve();
  });

  return dfd.promise;
}

function getApiMap(appName) {
  var dfd = Q.defer();
  mongo.apiMap.findOne(function(err, doc) {
    if (appName) {
      return dfd.resolve(doc[appName]);
    }
    return dfd.resolve(doc);
  });
  return dfd.promise;
}
