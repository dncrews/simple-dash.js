...
try {
  ...
  debug('Heroku client not configured'); // No penalty if Heroku configuration isn't defined
}
...
function _getRestart(appName, reason, cb) {
  ...
  return function doRestart() {
    ...
    heroku.apps(appName).dynos().restartAll(function restartHandler(err, resp) {
      if (err) {
        debug(err);
        if (cb) cb(err);
...
function sendNotification(appName, type, description, cb) {
  var dfd = Q.defer()
    , xEvents = {
      ...
      toWarning : 'Status change to Warning',
