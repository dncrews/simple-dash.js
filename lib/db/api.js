...
function save(doc) {
  ...
  db.status
    .save(doc, function(err, resp) {
      if (err) return debug('save Err: ', err) || dfd.reject(err);
...
function getCurrentByApp(appName) {
  ...
  getMap(appName).done(function(list) {
    db.status
      ...
      .limit(200, function(err, docs) {
        if (err) return debug('getCurrentByApp err: ', err) || dfd.reject(err);
...
function getHistory(apiName) {
  ...
  db.status
    ...
    .sort({ "timestamp" : -1 }, function(err, docs) {
      if (err) return debug('getHistory err: ', err) || dfd.reject(err);
...
function getMap(appName) {
  ...
  db.map.findOne(function(err, doc) {
    if (err) return debug('getMap err: ', err) || dfd.reject(err);
    if (appName) {
      list = doc[appName] || [];
...
function updateApiMap(list) {
  ...
  db.map.findOne(function(err, doc) {
    if (err) return debug('updateApiMap err: ', err) || dfd.reject(err);
