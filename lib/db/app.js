...
function getBucket(appName, timeBucket) {
  ...
  db.findOne({
    ...
    .sort({"timeBucket": -1},  function(err, doc) {
      if (err) return debug('getBucket err: ', err) || dfd.reject(err);
...
function saveBucket(doc) {
  ...
  db.save(doc, function(err, resp) {
    if (err) return debug('saveBucket err: ', err, doc) || dfd.reject(err);
...
function getHistory(appName) {
  ...
  db.find({
    ...
    .sort({ "timeBucket" : -1 }, function(err, docs) {
      if (err) return debug('getHistory err: ', err, appName) || dfd.reject(err);
...
function getCurrent() {
  ...
  db.find()
    ...
    .limit(200, function(err, docs) {
      if (err) return debug('getCurrent err: ', err) || dfd.reject(err);
...
function getPreviousStatus(appName, timeBucket) {
  ...
  db.findOne({
    ...
    .sort({ "timeBucket": -1 }, function(err, doc) {
      if (err) return debug('getPreviousStatus err: ', err, appName, timeBucket) || dfd.reject(err);
...
function createBucket(alertTitle, data, timeBucket) {
  ...
  db.findOne({
    ...
    .sort({"timeBucket": -1},  function(err, doc) {
      if (err) return debug('createBucket err: ', err, alertTitle, data, timeBucket) || dfd.reject(err);
      doc = doc || {
        "timeBucket" : timeBucket,
        "appName" : appName,
      ...
      db.save(doc, function(err, resp) {
        if (err) return debug('createBucket save err: ', err, doc) || dfd.reject(err);
...
function generateStats(doc) {
  ...
  if (!doc.stats.uptime_status) doc.stats.uptime_status = getStatus(doc);
