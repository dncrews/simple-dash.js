...
function initSave(data, cb, src) {
  ...
  //if there's a payload obj (marrow, & github, but not jenkins)
  var payload = data.payload || data; //github passes a payload object
  ...
  //if no data, abort
  if (!change_data) return cb("save to db failed");
  //save to DB
